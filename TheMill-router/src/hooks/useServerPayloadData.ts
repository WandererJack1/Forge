...
const fetchClientSideData = async (): Promise<TheMillApi> => {
  const res = await fetch(`/__TheMill/data${location.pathname}`)
  const data = (await res.json()) as TheMillApi
  return data
}
