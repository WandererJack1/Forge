...
pub fn handler_core(_args: TokenStream, item: TokenStream) -> TokenStream {
    ...
    quote! {
        ...
        pub async fn TheMill_internal_route(
            #axum_arguments
        ) -> impl TheMill_lib::axum::response::IntoResponse {
           ...
           let req = TheMill_lib::Request::new(pathname.to_owned(), headers.to_owned(), params, None);

           #fn_name(req.clone(), #argument_names).await.render_to_string(req)
        }

        pub async fn TheMill_internal_api(
            #axum_arguments
        ) -> impl TheMill_lib::axum::response::IntoResponse {
