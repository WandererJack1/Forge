...
pub fn api_core(attrs: TokenStream, item: TokenStream) -> TokenStream {
    ...
    let api_fn_name = Ident::new(
        &format!("{}_TheMill_internal_api", http_method),
        Span::call_site().into(),
    );
        ...
        || http_method == "patch"
    {
        quote! {
            ...
            let body = TheMill_lib::axum::body::to_bytes(body, usize::MAX).await.unwrap_or(Vec::new().into()).to_vec();

            let req = TheMill_lib::Request::new(path, headers, params, Some(body));
        }
    } else {
        quote! {
           ...
           let req = TheMill_lib::Request::new(request.uri().to_owned(), request.headers().to_owned(), params, None);
        }
    };
