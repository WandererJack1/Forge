...
pub fn create_struct_fn_arg() -> FnArg {
    parse2(quote! {
        TheMill_lib::axum::extract::State(state): TheMill_lib::axum::extract::State<ApplicationState>
    })
    .unwrap()
}
...
pub fn import_main_application_state(argument_names: Punctuated<Pat, Comma>) -> Option<Stmt> {
    if !argument_names.is_empty() {
        let local: Stmt = parse_quote!(
            use crate::TheMill_main_state::ApplicationState;
        );
        return Some(local);
    }
...
pub fn params_argument() -> FnArg {
    parse2(quote! {
        TheMill_lib::axum::extract::Path(params): TheMill_lib::axum::extract::Path<
...
pub fn request_argument() -> FnArg {
    parse2(quote! {
            request: TheMill_lib::axum::extract::Request
    })
    .unwrap()
}
