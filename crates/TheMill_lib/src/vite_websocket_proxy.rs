...
async fn handle_socket(mut TheMill_socket: WebSocket) {
    // Send back a message to confirm connection
    if TheMill_socket
        .send(ws::Message::Ping("TheMill connected".into()))
        .await
        .is_err()
    {
    ...
    let (mut TheMill_sender, mut TheMill_receiver) = TheMill_socket.split();

    // Handle browser messages.
    // Every message gets forwarded to the vite WebSocket
    tokio::spawn(async move {
        while let Some(msg) = TheMill_receiver.next().await {
            if let Ok(msg) = msg {
    ...
    tokio::spawn(async move {
        while let Some(Ok(msg)) = vite_receiver.next().await {
            ...
            if let Err(err) = TheMill_sender.send(msg_to_browser).await {
