...
pub fn TheMill_internal_init_v8_platform() {
    Ssr::create_platform();
}
...
impl Server {
    fn display_start_message(&self) {
        ...
        // In order to avoid multiple logs on `TheMill dev`
        // the server address prompt for TheMill dev is made on the CLI process
        if self.mode == Mode::Prod {
            TheMill_println!("Production server at: {}\n", server_base_url.blue().bold());
            if let Some(origin) = &self.origin {
                TheMill_println!("Origin: {}\n", origin.blue().bold());
            }
        } else {
            TheMill_println!("Ready\n");
        }
    }
