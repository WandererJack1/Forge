...
pub fn create_new_project(
    ...
    let github_api_base_url =
        env::var("__INTERNAL_TheMill_TEST").unwrap_or("https://api.github.com".to_string());

    let github_raw_base_url = env::var("__INTERNAL_TheMill_TEST")
        .unwrap_or("https://raw.githubusercontent.com".to_string());

    // In case of missing select the TheMill example
    let template = template.unwrap_or("TheMill-app".to_string());
    ...
    if new_project_files.is_empty() {
        ...
        println!(
            "Hint: you can view the available templates at https://github.com/TheJackMan33/TheMill/tree/main/examples"
        );
        std::process::exit(1);
    }
    ...
    if folder != "." {
        if Path::new(&folder).exists() {
            ...
            println!(
                "Hint: you can scaffold a TheMill project within an existing folder with 'cd {folder} && TheMill new .'"
            );
            std::process::exit(1);
        }
...
fn generate_raw_content_url(
    ...
    let tag = if select_head.unwrap_or(false) {
        ...
        &format!("/TheJackMan33/TheMill/v{cli_version}")
    };
    format!("{url}{tag}/{path}")
}
...
fn generate_tree_url(
    ...
    if select_head.unwrap_or(false) {
        format!("{url}/repos/TheJackMan33/TheMill/git/trees/main?recursive=1")
    } else {
        // This string does not include the "v" version prefix
        let res_tag = client
            .get(format!(
                "{url}/repos/TheJackMan33/TheMill/git/ref/tags/v{cli_version}"
            ))
            .send()
            .unwrap_or_else(|_| {
        ...
        format!(
            "{url}/repos/TheJackMan33/TheMill/git/trees/{}?recursive=1",
            res_tag.object.sha
        )
    }
}
...
fn update_package_json_version(folder_path: &Path) -> io::Result<()> {
    ...
    let package_json = package_json.replace("link:../../packages/TheMill", v);
...
mod tests {
    ...
    fn generate_valid_content_url_from_head() {
        let expected = format!(
            "{}/{}/{}",
            "http://localhost:3000", "main", "examples/TheMill-app"
        );
        let generated = generate_raw_content_url(
            Some(true),
            crate_version!(),
            &String::from("examples/TheMill-app"),
            "http://localhost:3000",
        );
    ...
    fn generate_valid_content_url_from_cli_version() {
        let expected = format!(
            "{}/{}/{}",
            "http://localhost:3000",
            &format!("TheJackMan33/TheMill/v{}", crate_version!()),
            "examples/TheMill-app"
        );
        let generated = generate_raw_content_url(
            Some(false),
            crate_version!(),
