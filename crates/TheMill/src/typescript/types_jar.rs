...
impl TypesJar {
    ...
    fn log_duplicated_types(&self) {
        ...
        for (type_name, file_paths) in duplicates.iter() {
            ...
            TheMill_println!("Duplicate \"{}\" type found in files:\n", type_name);
            TheMill_println!("- {}", first_file_path);
            TheMill_println!("- {}\n", second_file_path);
        }
    }
    ...
    fn generate_typescript(&self) -> String {
        ...
        let mut typescript = String::from("declare module \"TheMill/types\" {\n");
    ...
    pub fn generate_typescript_file(&mut self, base_path: &Path) -> std::io::Result<()> {
        ...
        let typescript_file_path = base_path.join(".TheMill").join("types.ts");
