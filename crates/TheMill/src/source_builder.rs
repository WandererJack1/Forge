...
#[cfg(not(target_os = "windows"))]
const MAIN_FILE_PATH: &str = "./.TheMill/main.rs";

#[cfg(not(target_os = "windows"))]
const FALLBACK_HTML_PATH: &str = "./.TheMill/index.html";

const ROUTE_FOLDER: &str = "src/routes";
const DEV_FOLDER: &str = ".TheMill";
...
// fix their setup to make TheMill work
...
impl SourceBuilder {
    pub fn new(mode: Mode) -> io::Result<Self> {
        if !PathBuf::from("TheMill.config.ts").exists() {
            recoverable_error("Cannot find TheMill.config.ts - is this a TheMill project?");
        }
    ...
    fn generate_axum_source(&self) -> String {
        ...
        let src = AXUM_ENTRY_POINT
            ...
            .replace(
                ...
                if app.has_app_state {
                    "let user_custom_state = TheMill_main_state::main();"
                } else {
                    ""
                },
            )
        ...
        for method in used_http_methods.into_iter() {
            ...
            import_http_handler.push_str(&format!("use TheMill_lib::axum::routing::{method};\n"))
        }
            ...
            if axum_info.is_some() {
                ...
                if !route.is_api() {
                    route_declarations.push_str(&format!(
                        r#".route("{axum_route}", get({module_import}::TheMill_internal_route))"#
                    ));

                    route_declarations.push_str(&format!(
                            r#".route("/__TheMill/data{axum_route}", get({module_import}::TheMill_internal_api))"#
                    ));
                } else {
                    for method in route.api_data.as_ref().unwrap().methods.clone() {
                        let method = method.to_string().to_lowercase();
                        route_declarations.push_str(&format!(
                                r#".route("{axum_route}", {method}({module_import}::{method}_TheMill_internal_api))"#
                        ));
                    }
                }
            }
...
mod tests {
    ...
    fn should_not_load_the_axum_get_function() {
        ...
        assert!(!dev_bundle.contains("use TheMill_lib::axum::routing::get;"));
    }
