...
fn read_http_methods_from_file(path: &String) -> Vec<Method> {
    let regex = Regex::new(r"TheMill_lib::api\((.*?)\)]").expect("Failed to create API regex");
...
impl Route {
    ...
    pub fn save_ssg_file(&self, reqwest: &Client) -> Result<(), String> {
        ...
        if self.axum_info.is_some() {
            ...
            let data_file_path = PathBuf::from(&format!("out/static/__TheMill/data{path}.json"));
            ...
            let base = Url::parse("http://localhost:3000/__TheMill/data").unwrap();

            let path = if path == "/" { "" } else { path };

            let pathname = &format!("/__TheMill/data{path}");
...
mod tests {
    ...
    fn should_find_dynamic_paths() {
        let routes = [
            ("/home/user/Documents/TheMill/src/routes/about.rs", false),
            ("/home/user/Documents/TheMill/src/routes/index.rs", false),
            (
